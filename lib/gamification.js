...
// ------------------------------------------------------------
// Duplication de séance, notes par exercice, mensurations, bilan
// périodique (migration_020).
// ------------------------------------------------------------

export async function duplicateLastSession(supabase, userId, programDayId) {
  const { data, error } = await supabase.rpc('duplicate_last_session', {
    p_user_id: userId,
...
export async function getPeriodRecap(supabase, userId, days = 30) {
  const { data, error } = await supabase.rpc('get_period_recap', {
    p_user_id: userId,
    p_days: days
  })
  if (error) throw error
...
export async function saveExerciseNote(supabase, sessionId, exerciseName, note) {
  const { error } = await supabase
    .from('session_exercise_notes')
    .upsert({ session_id: sessionId, exercise_name: exerciseName, note }, { onConflict: 'session_id,exercise_name' })
  if (error) throw error
}

export async function getExerciseNotes(supabase, sessionId) {
...
export async function saveBodyMeasurement(supabase, userId, { measuredAt, weightKg, measurements }) {
  const { error } = await supabase
...
export async function getBodyMeasurements(supabase, userId) {
  const { data, error } = await supabase
    .from('body_measurements')
    .select('measured_at, weight_kg, measurements')
    .eq('user_id', userId)
