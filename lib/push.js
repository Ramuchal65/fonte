...
export async function subscribeToPush(supabase, userId) {
  ...
  // Une fois refusée, la popup d'autorisation ne se redemande jamais toute
  // seule (comportement volontaire des navigateurs, anti-spam) —
  // requestPermission() renverrait juste 'denied' à nouveau sans rien
  // afficher. Autant le détecter avant et donner une instruction claire.
  if (Notification.permission === 'denied') {
    throw new Error('Notifications bloquées pour ce site — réautorise-les dans les paramètres du navigateur (icône ⓘ à côté de l\'adresse → Autorisations → Notifications), puis réessaie.')
  }
