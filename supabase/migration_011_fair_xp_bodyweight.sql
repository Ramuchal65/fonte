-- ============================================================
-- Correctif : le bonus XP se basait uniquement sur la charge totale
-- (Σ reps×kg), donc impossible à obtenir sur une séance au poids du
-- corps (0 kg partout) même en progressant clairement en répétitions.
--
-- Nouvelle logique : comparaison exercice par exercice, avec la
-- métrique adaptée à CHAQUE exercice — répétitions totales pour un
-- exercice au poids du corps (jamais de poids > 0 sur cet exercice
-- dans la séance), charge totale (Σ reps×kg) sinon. Le bonus est
-- proportionnel à la part d'exercices améliorés, plutôt que tout ou
-- rien sur un seul total global.
--
...
begin
  ...
  -- Comparaison exercice par exercice : répétitions pour le poids du corps
  -- (aucun poids > 0 loggé sur cet exercice), charge totale sinon.
  v_improved_count := 0;
    ...
    select
      ...
      count(*) filter (
        where p.exercise_name is not null and (
          ...
          or (not c.is_weighted and c.reps_total > p.reps_total)
        )
      )
